...
class RekaAdapter(BaseModelAdapter):
    ...
    def get_default_conv_template(self, model_path: str) -> Conversation:
        return get_conv_template("api_based_default")
class BetaModelAdapter(BaseModelAdapter):
    """The model adapter for adapted Zephyr-beta"""

    def match(self, model_path: str):
        return "beta" in model_path.lower()

    def get_default_conv_template(self, model_path: str) -> Conversation:
        return get_conv_template("zephyr")
class GemmaZephyrModelAdapter(BaseModelAdapter):
    """The model adapter for Gemma Zephyr"""

    def match(self, model_path: str):
        return "gemma" in model_path.lower()

    def get_default_conv_template(self, model_path: str) -> Conversation:
        return get_conv_template("h4_gemma_zephyr")

# Note: the registration order matters.
# The one registered earlier has a higher matching priority.
register_model_adapter(GemmaZephyrModelAdapter)
register_model_adapter(BetaModelAdapter)
